...
$("#rvizStart").click(function(event){

    const Url = "http://localhost:3000/rvizStart";
    $.post(Url,function(data,status){
        console.log(`${data} and Status is ${status}`);
        $("#rvizStatus").text("Connected");
        $("#rvizStart").attr("disabled", true);
        $("#rvizEnd").attr('disabled',false);
    });
    event.preventDefault();
});

$("#rvizEnd").click(function(event){
    $("#rvizEnd").attr('disabled',true);
    $("#rvizEnd").attr('value','Disconnecting');
    $("#rvizStatus").text("Disconnecting...");
    const Url = "http://localhost:3000/rvizEnd";
    $.post(Url,function(data,status){
        console.log(`${data} and Status is ${status}`);
        $("#rvizStatus").text("Closed");
        $("#rvizEnd").attr('value','End');
        $("#rvizStart").attr("disabled", false);
    });
    event.preventDefault();
});
