...
app.get('/Soil_analysis',function(req, res){
    res.render('index_2');
});

app.get('/Autonomous_Expedition',function(req, res){
    res.render('index_3');
});
...
var rviz;
app.post('/rviz:id',function(req,res){
    var id =  req.params.id;
    if(id=="Start"){
        mapviz= execFile('roslaunch',['rviz','rviz.launch'],function(err,stdout,stderr){
            if (err) {
                console.log(err);
            }
        });
        ...
        mapviz.on('exit',function(code){
            console.log(`rviz exit with ${code}`);
            res.send("Disconnected");
        });
    }
});
